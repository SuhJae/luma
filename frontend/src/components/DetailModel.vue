<template>
    <Teleport to="head">
        ...
        <meta property="og:url" :content="apiOrigin + 'api/v1/buildingurl/' + buildingData.url">
        <meta property="og:title" :content="buildingData.name + ' (' + palaceDict[selectedPalace] + ') - ' + langData.title">
        <meta property="og:description" :content="buildingData.explanation?.substring(0, 150)">
        <meta property="og:image" :content="apiOrigin + 'api/v1/media/' + buildingData.thumbnail">

        <!-- Twitter -->
        <meta property="twitter:card" content="summary_large_image">
        <meta property="twitter:url" :content="apiOrigin + 'api/v1/buildingurl/' + buildingData.url">
        <meta property="twitter:title" :content="buildingData.name + ' (' + palaceDict[selectedPalace] + ') - ' + langData.title">
        <meta property="twitter:description" :content="buildingData.explanation?.substring(0, 150)">
        <meta property="twitter:image" :content="apiOrigin + 'api/v1/media/' + buildingData.thumbnail">

        <!-- Link tags to other pages -->
        <link rel="alternate" hreflang="ko" :href="`https://luma.joseon.space/${palaceURL[selectedPalace]}/${buildingSlug}`" v-if="selectedPalace !== '0' && lang !== 'ko' && buildingSlug !== ''" />
        <link rel="alternate" hreflang="en" :href="`https://luma.joseon.space/${palaceURL[selectedPalace]}/${buildingSlug}`" v-if="selectedPalace !== '0' && lang !== 'en' && buildingSlug !== ''" />
        <link rel="alternate" hreflang="ja" :href="`https://luma.joseon.space/${palaceURL[selectedPalace]}/${buildingSlug}`" v-if="selectedPalace !== '0' && lang !== 'ja' && buildingSlug !== ''" />
        <link rel="alternate" hreflang="zh" :href="`https://luma.joseon.space/${palaceURL[selectedPalace]}/${buildingSlug}`" v-if="selectedPalace !== '0' && lang !== 'zh' && buildingSlug !== ''" />
        ...
        <component :is="'script'" type="application/ld+json">
            ...
            "image": [
            "{{ apiOrigin + 'api/v1/media/' + buildingData.thumbnail }}"
            ],
                ...
                <div class="w-full rounded-box bg-base-100 mt-20 mb-32 h-auto relative mx-auto max-w-3xl"
                    ...
                    <header class="mb-4">
                        <figure class="h-[50vh] overflow-hidden rounded-box rounded-b-none mb-8"
                            ...
                            <img v-if="buildingData.thumbnail" :src="apiOrigin + 'api/v1/media/' + buildingData.thumbnail"
                        ...
                        <div class="flex px-4 md:px-12 gap-2 pb-4">
                            ...
                            <div class="badge badge-primary drop-shadow-sm"
                                ...
                                {{ buildingData.detail_image.length }} {{ (buildingData.detail_image.length > 1) ?
                                    langData.photos_plural : langData.photos }} </div>
                            <div class="badge badge-primary drop-shadow-sm"
                                v-if="buildingData.detail_video && buildingData.detail_video.length > 0">
                                {{ buildingData.detail_video.length }} {{ (buildingData.detail_video.length > 1) ?
                                    langData.videos_plural : langData.videos }} </div>
                        </div>
                    </header>
                    ...
                    <article>
                        ...
                        <div class="carousel carousel-end w-full px-2 md:px-10 overflow-x-scroll overflow-y-visible pb-4"
                            ...
                            <div class="carousel-item" v-for="image in detailImages" :id="image.media">
                                <section
                                    ...
                                    <div :class="{ 'skeleton': !image.imageLoaded }" class="w-full h-2/3">
                                        <img :src="apiOrigin + 'api/v1/media/' + image.media"
                        ...
                        <div class="px-4 md:px-12" v-if="buildingData.explanation">
                            <span v-for="line in buildingData.explanation.split('\n')" class="mb-2">
                            ...
                            <div class="carousel-item" v-for="video in detailVideos" :key="video.video" :id="video.video">
                                <section @mouseover="hover = true" @mouseleave="hover = false"
                                    ...
                                    <div :class="{ 'skeleton': !video.Loaded }"
                                        ...
                                        <video :src="apiOrigin + 'api/v1/media/' + video.video" muted autoplay loop
...
.main-article {
    ...
    display: block;
    margin-bottom: 1rem;
    /* Adjust this value to control the space after line breaks */
}
...
const generateImageMeta = () => {
    ...
    detailImages.value.forEach((image) => {
        imageMeta.value.push({
            "@context": "https://schema.org/",
            "@type": "ImageObject",
            "contentUrl": apiOrigin + 'api/v1/media/' + image.media,
...
const generateVideoMeta = () => {
    ...
    detailVideos.value.forEach((video) => {
        videoMeta.value.push({
            ...
            "contentUrl": apiOrigin + 'api/v1/media/' + video.video,
            "embedUrl": apiOrigin + 'api/v1/media/' + video.video,
            "thumbnailUrl": apiOrigin + 'api/v1/media/' + video.video,
...
const fetchData = async (url, params) => {
    try {
        const response = await axios.get(apiOrigin + url, { params });
...
const fetchBuildingData = async () => {
    ...
    buildingData.value = await fetchData('api/v1/buildingurl/', {
        language: lang.value,
        building_name: props.building,
    });
    fetchDetailImages();
    fetchDetailVideos();
    updateMeta();
