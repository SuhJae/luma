<template>
    <div class="fixed inset-x-0 bottom-0 pb-10 flex justify-center px-2">
        <div class="flex min-w-fit max-w-screen rounded-full px-1 h-14 items-center bg-base-200 drop-shadow-lg backdrop-blur-md bg-opacity-80 bouncy-transition"
            :class="(isSearchExpanded) ? 'w-[35rem]' : 'w-1'">
            <!-- Search Suggestions -->
            <div class="dropdown dropdown-hover dropdown-end absolute bottom-full mb-2 min-w-fit max-w-screen bg-base-200 p-4 rounded-xl shadow-lg"
            ...
            <div class="flex-1" v-show="!isSearchExpanded">
                <div class="flex-none dropdown dropdown-top">
                    ...
                    <div class="backdrop-blur-md bg-opacity-80 dropdown-content z-[1] menu p-2 shadow rounded-box w-52 max-h-[60vh] overflow-y-auto grid bg-base-200"
                        tabindex="1">
...
const fetchAutocomplete = async () => {
    try {
        const response = await axios.get(apiOrigin + "api/v1/autocomplete/", {
            params: {
